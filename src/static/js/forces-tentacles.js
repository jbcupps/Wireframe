...
export function createTentacleGroup(scene, basePositions = null) {
  ...
  // Color palette tied to force type (will be overridden at runtime by update)
  const colors = [0x66ffaa, 0xffaa66, 0x88ddff];

  for (let i = 0; i < 3; i++) {
    // High-segment cylinder for visible twist and "knot" deformation
    ...
    // Store original geometry for dynamic deformation (knotting)
...
export function updateTentacles(tentGroup, params) {
  ...
  tentacles.forEach((tent, i) => {
    ...
    // Midpoint between defects (scaled by separation)
    ...
    // === Mode-specific twist & deformation math ===
    let twistAngle = twist;
    let extraRot = 0;
    let knotFactor = 0;

    if (mode === 'strong') {
      // Strong force: high twist + knotting/linking (CCP visualization)
      // Twist rate proportional to holonomy mismatch + compensation
      ...
      // Electroweak: symmetry-breaking twists (phase rotation + color flip)
      ...
      // EM: clean quantized flux tubes (Maxwell from topological flux)
...
/**
 * Applies a simple sinusoidal knotting deformation along the cylinder for strong-force mode.
 * This visually represents the topological linking required by the Causal Compensation Principle.
 */
function deformForKnotting(tent, intensity, time, index) {
  ...
  const yMax = 1.3; // half length in local space
  ...
  for (let j = 0; j < arr.length; j += 3) {
    ...
    // Parametric position along height
    const t = (y0 + yMax) / (2 * yMax); // 0..1

    // Two perpendicular sinusoidal waves with increasing frequency near center (knot)
...
/**
 * Helper: returns a suggested "glued" separation value when holonomy product is satisfied.
 * (Used by callers that also compute k and (-1)^k.)
 */
export function getGluedSeparation(holonomyProduct, ccp = 0.8) {
  // When product = +1 and high CCP → very close (strong knotting)
